...
//   const { inventoryId, phoneId, requestedQuantity } = req.body;

//   // Kiểm tra tính hợp lệ của inventoryId
...
//   // Kiểm tra tính hợp lệ của phoneId
//   if (!mongoose.Types.ObjectId.isValid(phoneId)) {
//     return res.status(400).json({
//       success: false,
...
//   // Kiểm tra số lượng yêu cầu phải lớn hơn 0
...
//     // Sử dụng phương thức checkStock từ schema để kiểm tra tồn kho
//     inventory.checkStock(phoneId, requestedQuantity);
...
//     // Tính toán availableStock từ quantity và reserved
//     const availableStock = product.quantity - product.reserved;
...
//       return res.status(400).json({
//         success: false,
//         message: "Không đủ tồn kho để xử lý đơn hàng.",
//       });
//     }
...
//     // Ghi lịch sử thay đổi
//     await inventory.addHistory("reserve", requestedQuantity, phoneId);
...
//   const { inventoryId, phoneId, completedQuantity } = req.body;
...
//     // Giảm reserved và quantity
//     if (product.reserved < completedQuantity) {
//       return res.status(400).json({
//         success: false,
//         message: "Số lượng hoàn tất vượt quá số lượng đặt trước.",
//       });
//     }

//     product.reserved -= completedQuantity;
//     product.quantity -= completedQuantity;

//     // Kiểm tra không để giá trị âm
//     if (product.reserved < 0 || product.quantity < 0) {
//       return res.status(400).json({
//         success: false,
...
//     // Ghi lịch sử thay đổi
//     await inventory.addHistory("purchase", completedQuantity, phoneId);
...
//   const { inventoryId, phoneId, canceledQuantity } = req.body;
...
//     // Giảm reserved
//     if (product.reserved < canceledQuantity) {
//       return res.status(400).json({
//         success: false,
//         message: "Số lượng hủy vượt quá số lượng đặt trước.",
//       });
//     }

//     product.reserved -= canceledQuantity;
...
//     // Ghi lịch sử thay đổi
//     await inventory.addHistory("unreserve", canceledQuantity, phoneId);
...
//     // Giả sử InventoryModel có lưu trữ lịch sử dưới dạng "history" array
//     // const inventory = await Inventory.findById(inventoryId, "history");
//     const inventory = await Inventory.findById(inventoryId)
//       .populate("phoneId", "name brand")
//       .select("+history");
...
//     //   "phoneId",
//     //   "name brand"
//     // ); // Lấy thêm thông tin sản phẩm nếu cần.
//     const inventory = await Inventory.findById(id).populate(
//       "phoneId",
//       "name brand price image"
//     );
...
//     // // Trả về thông tin tồn kho khả dụng (availableStock)
...
//     const inventory = await Inventory.findOne({ phoneId }).populate("phoneId");

//     if (!inventory) {
//       return res
//         .status(404)
//         .json({ message: "Sản phẩm không tồn tại trong tồn kho!" });
//     }

//     // Kiểm tra tồn kho khả dụng
//     const availableStock = inventory.quantity - inventory.reserved;
//     if (availableStock < requestedQuantity) {
//       // Tồn kho không đủ, gửi thông báo
//       return res.status(200).json({
//         message: "Sản phẩm hiện không đủ tồn kho!",
//         availableStock,
//         product: inventory.phoneId,
//       });
//     }
...
//     if (!phoneId || !quantity || quantity <= 0) {
//       return res.status(400).json({
//         success: false,
//         message: "Vui lòng cung cấp phoneId hợp lệ và số lượng lớn hơn 0.",
//       });
//     }
...
//     // Sử dụng phương thức checkStock để kiểm tra tồn kho
//     try {
//       targetInventory.checkStock(phoneId, quantity);
//     } catch (err) {
//       return res.status(400).json({
//         success: false,
//         message: err.message,
//         availableStock: targetProduct.quantity - targetProduct.reserved,
//       });
//     }

//     // Giảm quantity và tăng reserved
//     targetProduct.quantity -= quantity;
//     targetProduct.reserved += quantity;

//     // Ghi lịch sử thay đổi
//     await targetInventory.addHistory("purchase", quantity, phoneId);
